        ...
        async function uploadFile(uploader) {
            ...
            $.ajax({
                ...
                success : function(data) {
                    ...
                    const devide = $imageSources.val() ? ", " : "";
                    $imageSources.val($imageSources.val() + devide + data);
                    console.log( $imageSources.val());
                }
            });
        }
...
function uploadImage() {
    ...
    var uploader = $('<input id="fileUpload" name="fileUpload" type="file" accept="image/*" />')
    var images = $('.images')
    ...
    uploader.on('change', function () {
        ...
        uploadFile(uploader[0]);
    })

    images.on('click', '.img', function () {
        const imageSourcesArray = $("#imageSources").val().split(", ");
        const url = getRelativePath($(this).css("background-image").replace('url(','').replace(')','').replace(/\"/gi, ""));
        const index = imageSourcesArray.indexOf(url);
        if (index !== -1) {
            imageSourcesArray.splice(index, 1);
            $("#imageSources").val(imageSourcesArray.join(", "));
        }
        $(this).remove()
    })
